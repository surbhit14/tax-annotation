...
| [`renderer/render.ts`](renderer/render.ts) | Proof-of-concept renderer (TypeScript + pdf-lib) implementing the SPEC section 7 pipeline, typed against `types/annotation.ts`. |
...
└──────────┬───────────┘   └──────────┬───────────┘   └───────┬────────┘
           ...
           └──────────────────────────┼───────────────────────┘
                         ...
                         │ → format → draw        │   per the contract in SPEC.md section 7)
                         └────────────┬───────────┘
                                      ▼
                            Completed Form 1040
```
...
* **`binding`** reaches into the caller's nested data with a tiny JSONPath subset:
  `$.return.income.w2s`, `$.dependents[2].ssn` (SPEC section 6).
...
implementation just follows SPEC section 7: the six-step pipeline (resolve → default →
transform → validate → format → draw) over your PDF library of choice, converting
y-coordinates per SPEC section 3. The conformance checklist is SPEC section 9.
