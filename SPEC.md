...
4. **Fail loudly where money is involved.** Silent truncation of a dollar amount is worse
   than an error. Defaults are chosen accordingly (see sections 7.4 and 5.8).
...
trace the printed box on the form; the renderer handles insetting via `padding` (section 4).
...
| `overflow`    | `"shrink"`    | See section 7.4. |
...
`align: "right"`, section 5.3).

## 5. Field types

All fields share the base properties: `id` (unique in the document; convention is the
form's own line label, e.g. `"line-1a"`), `label`, `page`, `binding` (section 6), `required`,
`default`, `transforms`, `style`, `notes`.

`required`/`default` semantics: if the binding resolves to *missing* (section 6.3):
...
Free text in a rect. `multiline: true` enables word-wrapping (section 7.3); `case: "upper"`
...
* `fit`: with `"exact"` (the default), after stripping, the value's length MUST equal
  `cells`; a mismatch is a render error (a 7-digit "SSN" must never print). `"left"`: the
  length MUST be ≤ `cells`; cells fill from cell 1 and the remainder stay blank, the
  right behavior for bank account numbers, which the 1040 gives 17 boxes.
...
* Inside columns, bindings use `@`, the current entry's array element (section 6.2).
...
index all resolve to **missing** (not an error at resolution time; see section 6.3). A resolved
JSON `null` is treated as missing.

### 6.3 Missing values

Missing flows into the `required`/`default` logic of section 5: error if `required`, else
...
| fn | Args | Effect |
|----|------|--------|
| `abs` | (none) | Absolute value of a number. |
| `negate` | (none) | Multiply by −1. |
...
| `uppercase` / `lowercase` | (none) | Case-map a string. |
| `trim` | (none) | Strip surrounding whitespace. |
...
1. **Resolve** the binding (section 6.2); for tables, resolve the array, then process each row.
2. **Apply** `required`/`default` (section 6.3); skip the field if it comes up empty.
3. **Transform** (section 6.4).
...
5. **Format** to a string (section 7.2); not applicable to checkbox/radio/fill marks.
6. **Lay out and draw** within the padded rect (sections 7.3 and 7.4), converting
   coordinates to the PDF library's space (section 3).
...
* **number/currency:** round to `decimals` (half away from zero), group thousands with
  ...
  policy, `"print"` → `0` (or `0.00`), `"blank"` → skip the field, `"dash"` → an em dash.
* **date:** apply `pattern` tokens (section 5.4).
...
When formatted text does not fit (section 7.1) at the effective font size:
...
implements section 3 coordinate conversion; resolves the full section 6.1 grammar;
implements all section 6.4 transforms; implements every section 5 field type with the
section 7 pipeline; and reports every
